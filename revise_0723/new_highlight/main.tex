...
\hladd{The policy mean specifies the nominal three-axis acceleration
...
The complete ART-MAPPO workflow, integrating all three modules described above, is illustrated in Fig.~\ref{fig:algorithm_flow}. \hladd{The corresponding training procedure under the CTDE paradigm is detailed in Algorithm~\ref{alg:artmappo}.}
...
ART-MAPPO employs the piecewise Dual-Clip objective in
...
\hldel{communication delays,} limited bandwidth and heterogeneous UAV capabilities.
...
\appendices
\section{\hladd{Summary of Key Variables and Symbols}}
\label{app:notation}

\hladd{Table~\ref{tab:notation} consolidates the principal variables and symbols used throughout the paper.}

\begin{table}[!t]
\centering
\caption{\hladd{Key Variables and Symbols in Sections II--IV}}
\label{tab:notation}
{\color{blue}
\scriptsize
\setlength{\tabcolsep}{1.8pt}
