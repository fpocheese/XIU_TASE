...
\subsection{Offline Reward-Function Sensitivity Analysis}
\label{subsec:offline_reward_sensitivity}

We further examine the sensitivity of the reward definition to three representative weights
...
\eqref{eq:reward_function}, respectively, while
...
This experiment isolates reward-function sensitivity from policy adaptation. Specifically,
...
parameter setting is then evaluated by recomputing the reward on these \emph{same} state--action
trajectories. For episode $e$, the normalized cumulative-reward variation is
\begin{equation}
\label{eq:normalized_reward_sensitivity}
\Delta G_{e}^{\mathrm{norm}}(\kappa;\theta)=
\frac{G_e(\kappa\theta_0)-G_e(\theta_0)}
{\sum_t|r_e(t;\theta_0)|+10^{-8}},
\end{equation}
where $G_e(\theta)=\sum_t r_e(t;\theta)$. Thus, Fig.~\ref{fig:reward_sensitivity}
...
\caption{Sensitivity of the reward function to representative weighting and shaping
...
The component-level responses in Figs.~\ref{fig:reward_sensitivity}(a)--(c) agree with the
