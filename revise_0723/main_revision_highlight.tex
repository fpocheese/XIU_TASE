...
%DIF DELETED-DISPLAY-MATH: red formula with a horizontal strike through its vertical centre %DIF PREAMBLE
\newsavebox{\DIFDELbox} %DIF PREAMBLE
\newcommand{\DIFdelDISPLAY}[1]{% %DIF PREAMBLE
  \par\addvspace{4pt}% %DIF PREAMBLE
  \sbox{\DIFDELbox}{$\displaystyle\color{red}\begin{aligned}#1\end{aligned}$}% %DIF PREAMBLE
  \noindent\hbox to\hsize{\hfil %DIF PREAMBLE
    \rlap{\usebox{\DIFDELbox}}% %DIF PREAMBLE
    \raisebox{\dimexpr(\ht\DIFDELbox-\dp\DIFDELbox)/2\relax}[0pt][0pt]{\textcolor{red}{\rule{\wd\DIFDELbox}{0.6pt}}}% %DIF PREAMBLE
    \hfil}% %DIF PREAMBLE
  \par\addvspace{4pt}% %DIF PREAMBLE
} %DIF PREAMBLE
...
% ---- Float-placement tuning (revision highlight): let several figures share a page ----
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=2.5in]{KM1.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
...
& \quad + \beta \cdot \mathbf{r}_i \cdot \exp\left(-\frac{t}{T}\right)  \\
\text{Dancing:} \quad & \mathbf{x}_i^{(t+1)} = \mathbf{x}_i^{(t)} \odot \exp\left(\gamma \cdot \tanh(\mathbf{A}_i - \bar{A}_i)\right)  \\
& \quad + \mathbf{n}_i, \quad \mathbf{n}_i \sim \mathcal{N}(0, \sigma_d^2 \mathbf{I})  \\
...
& \quad + \delta' \cdot \left(\mathbf{x}_{\text{second}}^{(t)} - \mathbf{x}_i^{(t)}\right)  \\
\text{Stealing:} \quad & \mathbf{x}_i^{(t+1)} = \mathbf{x}_i^{(t)}  \\
& \quad + \eta \cdot \sum_{k \in \mathcal{K}_i} \frac{A_{ij}}{\sum_{l \in \mathcal{K}_i} A_{lj}} \cdot \left(\mathbf{x}_k^{(t)} - \mathbf{x}_i^{(t)}\right)
}
...
Concretely, every exploratory coefficient follows the linear schedule
$c_k=c_0(1-k/K_{\mathrm{IDBO}})$ for $c\in\{\alpha,\beta,\gamma,\delta,\delta',\eta\}$ and the
dancing noise scale $\sigma_d$, where $K_{\mathrm{IDBO}}$ is the iteration budget; the elite
recombination weights stay bounded. This schedule governs convergence stability: because the
stochastic parts of the rolling and dancing operators scale with $c_k$ and vanish as
$c_k\!\to\!0$, the update degenerates into a deterministic, advantage-weighted local
refinement composed with the projection $\Pi_{\Xi}$, which is a Robbins--Monro-type sufficient
condition for settling at a stable fixed point rather than orbiting it. An ablation that
freezes the coefficients (constant, or no decay) confirms this: non-decaying coefficients leave
a persistent late-stage oscillation in the working assignment, whereas the linear schedule
drives the swarm to a stable, low-cost equilibrium.
...
%DIFDELCMD < %%%
\DIFdelDISPLAY{%
\text{Phase 1: } & \mathbf{x}_i^{(k+1)} = \mathcal{F}_{\text{optimizer}}\left(\mathbf{x}_i^{(k)}, \mathcal{F}_i, \mathbf{A}_i^{(k)}, \mathbf{P}^{(k)}\right)  \\
\text{Phase 2: } & \{\mathbf{z}_i^{(k+1)}, \mathcal{B}_i^{(k+1)}\} =  \\ 
& \mathcal{G}_{\text{consensus}}\left(\{\sigma(\mathbf{x}_i^{(k+1)})\}, \{\mathbf{c}_i^{(k+1)}\}, \{\mathbf{y}_i^{(k)}\}\right)  \\
\text{Phase 3: } & \mathbf{A}_i^{(k+1)} = \mathcal{H}\left(\mathbf{z}_i^{(k+1)}, \mathcal{B}_i^{(k+1)}, \mathbf{V}_i^{(k+1)}, \mathbf{V}_T^{(k+1)}\right)
}
...
Since $N_A$ and the neighborhood size $|\mathcal N_i|$ are fixed by the engagement and the
communication graph rather than by the swarm size, the per-defender cost is independent of
$N_D$ and the total per-round cost grows linearly with $N_D$. Empirically, the time to reach
consensus is governed by the communication-graph diameter $D$ and the communication delay
rather than by $N_D$: it stays nearly constant as the swarm scales, and it grows only linearly
with both the communication delay and $D$, consistent with the $O(N\cdot D)$ bound. A bounded
communication delay therefore slows, but does not prevent, convergence of the distributed
consensus.
}
...
\mathbf{A}_j &= \mathrm{softmax}\!\left(\frac{\mathbf{Q}_j\mathbf{K}_j^\top}{\sqrt{d_k}} + \mathbf{M}\right) \in \mathbb{R}^{n_o \times n_o} 
\\
...
\mathbf{z}_1^{(\ell)} &= \operatorname{ReLU} \left( \operatorname{LN}_1^{(\ell)} \left( \mathbf{h}^{(\ell)}\mathbf{W}_1^{(\ell)} + \mathbf{b}_1^{(\ell)} \right) \right)  \\
\mathbf{z}_2^{(\ell)} &= \operatorname{LN}_2^{(\ell)} \left( \mathbf{z}_1^{(\ell)}\mathbf{W}_2^{(\ell)} + \mathbf{b}_2^{(\ell)} \right)  \\
\mathbf{h}^{(\ell+1)} &= \operatorname{ReLU} \left( \mathbf{z}_2^{(\ell)} + \mathbf{h}^{(\ell)} \right)
}
...
\mu_R &\leftarrow (1-\rho)\,\mu_R + \rho\,\frac{1}{N}\sum_{i=1}^{N} R_i^{(k)}  \\
\sigma_R^2 &\leftarrow (1-\rho)\,\sigma_R^2 + \rho\,\frac{1}{N}\sum_{i=1}^{N}\!\left(R_i^{(k)} - \mu_R\right)^2
}
...
opposite. Equation~\eqref{eq:trust_update} is a convergent exponential-smoothing tracker,
not a monotonic accumulator: for a stationary $\widetilde G_i^{(k)}$ its unique fixed point
is $\mathcal T_i^{\star}=\sigma(\tau_T\widetilde G_i^{(k)})$, so trust tracks the smoothed
relative performance rather than growing without bound. Since $\widetilde G_i^{(k)}$ is
normalized against the swarm baseline, a converged swarm gives $\widetilde G_i^{(k)}\!\to\!0$
and thus a residual exploration floor $\beta_i=0.5$ during training, which is intentionally
removed at deployment ($\beta_i=0$).
}\DIFaddend
...
\hat{A}_t &= \sum_{l=0}^{T-t-1} (\gamma_{\mathrm{RL}}\lambda_{\mathrm{GAE}})^l \delta_{t+l}  \\
\delta_t &= r_i^{(t)} + \gamma_{\mathrm{RL}} V_\phi(s_{t+1}) - V_\phi(s_t)
}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=7.0in]{rlsuanfa.pdf}
...
r_i^{\text{dist}} &= \exp\!\left(-\alpha_d \left\|\mathbf{p}_i^{(t)} - \mathbf{p}_{\text{tgt}}\right\|\right) \\
r_i^{\text{angle}} &= -\alpha_a \left|\gamma_{D_i} - q_i\right| \\
r_i^{\text{hit}} &= R_{\text{hit}} \cdot \mathbb{I}\!\left(\left\|\mathbf{p}_i^{(t)} - \mathbf{p}_{\text{tgt}}\right\| < d_{\text{kill}}\right) \\
r_i^{\text{coord}} &= -\alpha_c \left| t_{go_i}^{(t)} - \bar{t}_{go}^{(t)} \right|, \quad \bar{t}_{go}^{(t)} = \frac{1}{|\mathcal{G}_i|}\sum_{j \in \mathcal{G}_i} t_{go_j}^{(t)} \\
r_i^{\text{energy}} &= -\alpha_e \left(n_{x_i}^2 + n_{y_i}^2\right)
}
...
Fig.~\ref{fig:convergence} illustrates the statistical performance in terms of average cost convergence over iterations. IDBO significantly outperforms the baselines, reaching the lowest steady-state cost within 100~iterations. \begin{figure}[!tbp] 
\centering 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.15in]{../new_sim_fig/figures_v10/mappo/mappo_nopn_trajectory_3d.png}
...
acceleration (}\DIFdelend \DIFaddbegin \begin{figure}[!tbp]
\centering
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.2in]{../new_sim_fig/figures_v10/mappo/mappo_nopn_tgo.png}
...
Fig.~\ref{fig:case1_pn_traj} andFig.~\ref{fig:case1_pn_kine}. While PN eventually intercepts the targets (Fig.~\ref{fig:case1_pn_traj}), }\DIFdelend \DIFaddbegin \begin{figure}[!tbp]
\centering
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.15in]{../new_sim_fig/figures_v10/pn/pn_nopn_trajectory_3d.png}
...
%DIFDELCMD < \vspace{0pt}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.15in]{../new_sim_fig/figures_v10/pn/pn_nopn_trajectory.png}
...
as evidenced by the time-to-go ($t_{go}$) profiles in Fig.~\ref{fig:case1_mappo_tgo}. The }\DIFdelend \DIFaddbegin \begin{figure}[!tbp]
\centering
\includegraphics[width=3.1in]{../new_sim_fig/figures_v10/pn/standalone_duav_legend.png}\\[-2pt]
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.2in]{../new_sim_fig/figures_v10/pn/pn_nopn_tgo.png}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.0in]{../new_sim_fig/figures_v10/pn/pn_nopn_time_sync.png}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.15in]{../new_sim_fig/figures_v10/mappo/mappo_sin_trajectory_3d.png}
...
%DIFDELCMD < \vspace{0pt}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.1in]{../new_sim_fig/figures_v10/mappo/standalone_duav_legend.png}\\[-2pt]
...
\DIFdelbegin \DIFdel{Conversely, the PN baseline completely fails to handle the high-frequency dynamic engagement geometry. The reactive nature of PN results in phase-lagged trajectories that struggle to close the distance }\DIFdelend \DIFaddbegin \begin{figure}[!tbp]
\centering
\includegraphics[width=3.1in]{../new_sim_fig/figures_v10/mappo/standalone_duav_legend.png}\\[-2pt]
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.2in]{../new_sim_fig/figures_v10/mappo/mappo_sin_tgo.png}
...
%DIFDELCMD < \vspace{0pt}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.0in]{../new_sim_fig/figures_v10/mappo/mappo_sin_time_sync.png}
...
\DIFdelbegin \DIFdel{The continuous maneuvering of the targets also disrupts conventional coordination mechanisms. However, this vital temporal synchronization is dynamically sustained by ART-MAPPO during the flight, as evidenced by the }\DIFdelend \DIFaddbegin \begin{figure}[!tbp]
\centering
\includegraphics[width=3.1in]{../new_sim_fig/figures_v10/pn/standalone_duav_legend.png}\\[-2pt]
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.2in]{../new_sim_fig/figures_v10/pn/pn_sin_tgo.png}
\DIFaddendFL \caption{Time-to-go ($t_{go}$) profiles of the defending swarm using \DIFdelbeginFL \DIFdelFL{ART-MAPPO }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the PN baseline }\DIFaddendFL in
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=3.0in]{../new_sim_fig/figures_v10/pn/pn_sin_time_sync.png}
\caption{\DIFaddFL{Terminal arrival time differences $\Delta t$ within each PN interceptor group in
Case~2.}}
...
\DIFdelendFL \DIFaddbeginFL \subfloat[Case~1]{\includegraphics[width=3.47in]{nopn_mc_boxplot_compare.png}}\DIFaddendFL \\
\DIFdelbeginFL %DIFDELCMD < \subfloat[Case~2]{\includegraphics[width=3.5in]{sin_mc_boxplot_compare.png}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \subfloat[Case~2]{\includegraphics[width=3.47in]{sin_mc_boxplot_compare.png}}
