...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        self.reward_definition = str(
            _arg(args, "reward_definition", "legacy")
        ).lower()
        ...
        self.attack_pattern = str(
            _arg(args, "attack_pattern", "nominal")
        ).lower()
        self.unseen_chirp_rate = float(
            _arg(args, "unseen_chirp_rate", 0.004)
        )
        self.unseen_frequency_scale = float(
            _arg(args, "unseen_frequency_scale", 1.35)
        )
        self.unseen_vertical_load_amp = float(
            _arg(args, "unseen_vertical_load_amp", 0.18)
        )
        self.case3_terminal_frequency_scale = float(
            _arg(args, "case3_terminal_frequency_scale", 0.45)
        )
        self.case3_terminal_chirp_rate = float(
            _arg(args, "case3_terminal_chirp_rate", 0.0008)
        )
        self.case3_terminal_frequency_cap = float(
            _arg(args, "case3_terminal_frequency_cap", 0.16)
        )
        self.case4_bangbang_period = float(
            _arg(args, "case4_bangbang_period", 4.2)
        )
        self.case4_bangbang_min_dwell = float(
            _arg(args, "case4_bangbang_min_dwell", 1.6)
        )
        self.case4_bangbang_hysteresis = float(
            _arg(args, "case4_bangbang_hysteresis", 0.15)
        )
        self.case4_bangbang_axial_load = float(
            _arg(args, "case4_bangbang_axial_load", 0.10)
        )
        self.case4_bangbang_lateral_scale = float(
            _arg(args, "case4_bangbang_lateral_scale", 0.75)
        )
        ...
        self.trust_pn_navigation_constant = float(
            _arg(args, "trust_pn_navigation_constant", 3.0)
        )
        ...
        world.trust_los_rate_filter_alpha = _arg(
            args, "trust_los_rate_filter_alpha", 0.20
        )
        world.trust_los_rate_window_steps = int(
            _arg(args, "trust_los_rate_window_steps", 21)
        )
        ...
        world.physical_episode_horizon_steps = int(
            _arg(args, "physical_episode_horizon_steps", 1500)
        )
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            agent.state.defender_previous_observed_los = None
            agent.state.defender_observed_los_history = []
            agent.state.defender_observed_los = None
            agent.state.defender_observed_los_rate = np.zeros(3)
            agent.state.defender_filtered_los_rate = np.zeros(3)
            agent.state.defender_last_observed_closing = 0.0
            agent.state.defender_last_tgo_error = 0.0
            ...
            agent.state.env_time_limit = False
            ...
            agent.state.bangbang_lateral_sign = None
            agent.state.bangbang_vertical_sign = None
            agent.state.bangbang_last_switch_t = -np.inf
    ...
    def _paper_key(self):
        # Cases 3 and 4 are frozen-policy/generalization conditions.  They start
        # from the Case-2 paper preset as a reproducible state template; the
        # evaluator then applies the preregistered transformed geometry before
        # assignment, and the attacker uses a distinct maneuver below.
        return "case2" if self.case in {"case2", "case3", "case4"} else "case1"
    ...
    def adversary_reward(self, agent, world):
        if agent.doneflag:
            ...
            if getattr(agent.state, "actual_hit", False):
                ...
                if not getattr(agent.state, "hit_reward_paid", False):
                    ...
                    if self.reward_definition != "paper_five_component":
                        terminal -= self.reward_w_sync * self._async_terminal_penalty(agent, world)
                if (
                    self.reward_definition != "paper_five_component"
                    and not getattr(agent.state, "sync_reward_paid", False)
                ):
        ...
        if self.reward_definition == "paper_five_component":
            r_angle = -self.reward_alpha_angle * angle_err
        else:
            r_angle = -self.reward_alpha_angle * (
                angle_err ** self.reward_angle_power
            )

        # First entry into the kill radius is paid in the done-state branch.
        # No dense hit shaping is part of the paper's five-component reward.
        r_hit = 0.0
        if (
            self.reward_definition != "paper_five_component"
            and self.reward_hit_shaping > 0.0
        ):
            band = max(self.hit_radius * self.reward_hit_band_ratio, self.hit_radius + 1e-6)
            r_hit += self.reward_hit_shaping * np.exp(-dist / band)
        if self.reward_definition == "paper_five_component":
            r_energy = -self.reward_alpha_energy * np.sum(
                np.square(agent.state.load)
            )
        else:
            r_energy = -self.reward_alpha_energy * np.sum(
                np.square(agent.action.u)
            ) * world.dt
        ...
        if self.reward_definition == "paper_five_component":
            r_coord = -self.reward_alpha_coord * time_abs
        else:
            r_coord = -self.reward_alpha_coord * (
                time_abs ** self.reward_coord_power
            )
        if (
            self.reward_definition != "paper_five_component"
            and time_abs <= self.reward_coord_tol
        ):
    ...
    def observation(self, agent, world):
        ...
        # Preserve the exact delayed/noisy packet used to form the actor
        # observation. The training guide must use this same information set;
        # otherwise it becomes a privileged teacher that the decentralized
        # actor cannot reproduce at deployment.
        agent.state.defender_last_observed_target_pos = np.asarray(
            target_pos, dtype=float
        ).copy()
        agent.state.defender_last_observed_target_vel = np.asarray(
            target_vel, dtype=float
        ).copy()
        ...
        # The paper's PN exploration component is defined from a measured
        # LOS rate, whereas the 17-D decentralized observation intentionally
        # does not expose the target-velocity vector.  Estimate the guide's
        # LOS rate from the same delayed/noisy LOS sequence seen by the GRU,
        # so the guide cannot exploit transverse-velocity information that
        # the deployed actor can never reconstruct from its current input.
        los_history = list(
            getattr(
                agent.state,
                "defender_observed_los_history",
                [],
            )
        )
        los_history.append(
            (float(agent.state.timestep), los.copy())
        )
        window_steps = max(
            2,
            int(
                getattr(
                    world, "trust_los_rate_window_steps", 21
                )
            ),
        )
        if len(los_history) > window_steps:
            los_history = los_history[-window_steps:]
        agent.state.defender_observed_los_history = los_history
        if len(los_history) < 2:
            los_rate = np.zeros(3, dtype=float)
        else:
            # Least-squares slope over the actor-visible LOS history is much
            # less sensitive to the 3-m position noise than a one-step
            # difference at 20 Hz.  The GRU receives this same history.
            times = np.asarray(
                [item[0] for item in los_history], dtype=float
            )
            samples = np.asarray(
                [item[1] for item in los_history], dtype=float
            )
            ...
            los_rate -= float(np.dot(los_rate, los)) * los
        filter_alpha = float(
            np.clip(
                getattr(world, "trust_los_rate_filter_alpha", 0.20),
                0.0,
                1.0,
            )
        )
        previous_filtered = np.asarray(
            getattr(
                agent.state,
                "defender_filtered_los_rate",
                np.zeros(3),
            ),
            dtype=float,
        )
        filtered_los_rate = (
            (1.0 - filter_alpha) * previous_filtered
            + filter_alpha * los_rate
        )
        agent.state.defender_previous_observed_los = los.copy()
        agent.state.defender_observed_los = los.copy()
        agent.state.defender_observed_los_rate = los_rate.copy()
        agent.state.defender_filtered_los_rate = (
            filtered_los_rate.copy()
        )
        agent.state.defender_last_observed_closing = float(closing)
        flight_dir = self._flight_dir(agent)
        angle_err = np.arccos(np.clip(np.dot(flight_dir, los), -1.0, 1.0))
        tgo = dist / max(agent.state.v_vel[0], 1.0)
        ...
        agent.state.defender_last_tgo_error = float(tgo - tgo_mean)
    ...
    def done_callback(self, agent, world):
        ...
        if agent.adversary and self.is_collision(agent, world.agents[agent.target]):
            ...
            return True
        horizon = max(
            int(getattr(world, "physical_episode_horizon_steps", 1500)),
            1,
        )
        elapsed_steps = int(
            round(
                float(agent.state.timestep)
                / max(float(world.dt), 1e-12)
            )
        )
        if elapsed_steps >= horizon:
            agent.doneflag = True
            agent.state.actual_hit = False
            agent.state.env_time_limit = True
            return True
        return False
    ...
    def _paper_attacker_action(self, agent, world):
        ...
        if pattern != "nominal":
            # Frozen-policy generalization patterns.  These are deliberately
            # absent from training and preserve the target's terminal PN
            # tendency toward the defended asset while changing only the
            # exogenous maneuver waveform.
            sign = preset_scalar(
                f"{key}_sin_sign" if key == "case2" else "case1_nopn_sign",
                1.0,
            )
            N = preset_scalar(
                f"{key}_sin_N" if key == "case2" else "case1_nopn_N",
                3.0,
            )
            omega = preset_scalar(
                "case2_sin_omega", self.case2_maneuver_freq
            )
            omega *= self.unseen_frequency_scale
            amp = preset_scalar(
                "case2_sin_A", 9.8 * self.case2_lateral_amp
            )
            phase = (
                preset_scalar("case2_sin_phase", 0.0)
                + 2.0 * np.pi * j / max(len(self.good_agents(world)), 1)
            )
            fade = np.clip(
                horizontal_dist
                / max(float(self.attack_maneuver_fade_range), 1.0),
                0.0,
                1.0,
            )
            ...
            if pattern == "chirp":
                wave_phase = (
                    omega * agent.state.timestep
                    + 0.5
                    * self.unseen_chirp_rate
                    * agent.state.timestep ** 2
                    + phase
                )
                lateral_wave = np.sin(wave_phase)
                vertical_wave = np.sin(0.61 * wave_phase + 0.37 * phase)
            elif pattern == "multisine":
                t = agent.state.timestep
                lateral_wave = (
                    0.65 * np.sin(1.37 * omega * t + phase)
                    + 0.35 * np.sin(0.63 * omega * t - 0.5 * phase)
                )
                vertical_wave = np.sin(0.83 * omega * t + 0.7 * phase)
            elif pattern == "jink":
                t = agent.state.timestep
                lateral_wave = np.tanh(
                    4.0 * np.sin(0.72 * omega * t + phase)
                )
                vertical_wave = np.tanh(
                    3.0 * np.sin(0.49 * omega * t + 0.7 * phase)
                )
            elif pattern == "case3_hybrid":
                # Unseen heterogeneous switching strategy for reviewer Case 3.
                # The eight attackers use three phase-shifted roles and change
                # from multisine weaving to bang-bang jinks and finally a
                # frequency-swept terminal penetration.  The longitudinal
                # pulse makes this condition different from both the
                # Case-1 maneuver-to-PN transition and Case-2 fixed-frequency
                # continuous weaving.
                t = agent.state.timestep
                role = j % 3
                switch_1 = 12.0 + 1.5 * role
                switch_2 = 29.0 + 2.0 * role
                if t < switch_1:
                    lateral_wave = (
                        0.68 * np.sin(1.31 * omega * t + phase)
                        + 0.32 * np.sin(0.47 * omega * t - 0.6 * phase)
                    )
                    vertical_wave = np.sin(
                        0.73 * omega * t + 0.45 * phase
                    )
                    axial_wave = 0.18 * np.sin(
                        0.39 * omega * t + 0.3 * phase
                    )
                elif t < switch_2:
                    lateral_wave = np.tanh(
                        5.0
                        * np.sin(
                            (0.58 + 0.06 * role) * omega * t
                            + phase
                            + np.pi * (role % 2)
                        )
                    )
                    vertical_wave = np.tanh(
                        4.0
                        * np.sin(
                            (0.41 + 0.04 * role) * omega * t
                            + 0.8 * phase
                        )
                    )
                    axial_wave = -0.22 if role == 1 else 0.22
                else:
                    tau = max(0.0, t - switch_2)
                    terminal_omega = max(
                        1e-6,
                        self.case3_terminal_frequency_scale * omega,
                    )
                    terminal_cap = max(terminal_omega, self.case3_terminal_frequency_cap)
                    terminal_omega = min(terminal_omega, terminal_cap)
                    chirp_rate = max(
                        0.0,
                        min(self.case3_terminal_chirp_rate, self.unseen_chirp_rate),
                    )
                    if chirp_rate > 0.0 and terminal_cap > terminal_omega:
                        tau_cap = (terminal_cap - terminal_omega) / chirp_rate
                        if tau <= tau_cap:
                            terminal_phase = terminal_omega * tau + 0.5 * chirp_rate * tau ** 2
                        else:
                            terminal_phase = (
                                terminal_omega * tau_cap
                                + 0.5 * chirp_rate * tau_cap ** 2
                                + terminal_cap * (tau - tau_cap)
                            )
                    else:
                        terminal_phase = terminal_omega * tau
                    wave_phase = terminal_phase + phase
                    lateral_wave = np.sin(wave_phase)
                    vertical_wave = np.sin(
                        0.47 * wave_phase + 0.5 * phase
                    )
                    axial_wave = 0.16 * np.sin(
                        0.23 * wave_phase + phase
                    )
                load_x += fade * axial_wave
            elif pattern == "bangbang":
                # Case-4 attacker: bounded bang-bang evasion with finite dwell.
                # The sign is only allowed to change after a minimum dwell time,
                # which prevents unrealistic high-frequency chattering while
                # preserving the intended upper/lower-bound switching behavior.
                t = agent.state.timestep
                role = j % 4
                base_period = max(2.0 * self.case4_bangbang_min_dwell, self.case4_bangbang_period)
                period = base_period + 0.35 * (role - 1.5)
                phase_role = phase + 0.55 * role
                switch_signal = (
                    np.sin(2.0 * np.pi * t / period + phase_role)
                    + 0.25 * np.sin(2.0 * np.pi * t / (1.75 * period) - 0.4 * phase_role)
                )
                raw_lateral = 1.0 if switch_signal >= 0.0 else -1.0
                current_lateral = getattr(agent.state, "bangbang_lateral_sign", None)
                last_switch = float(getattr(agent.state, "bangbang_last_switch_t", -np.inf))
                if current_lateral is None:
                    current_lateral = raw_lateral
                    agent.state.bangbang_last_switch_t = t
                elif (
                    raw_lateral != current_lateral
                    and t - last_switch >= self.case4_bangbang_min_dwell
                    and abs(switch_signal) >= self.case4_bangbang_hysteresis
                ):
                    current_lateral = raw_lateral
                    agent.state.bangbang_last_switch_t = t
                agent.state.bangbang_lateral_sign = current_lateral

                vertical_signal = np.sin(
                    2.0 * np.pi * t / (1.45 * period) + 0.7 * phase_role
                )
                current_vertical = getattr(agent.state, "bangbang_vertical_sign", None)
                raw_vertical = 1.0 if vertical_signal >= 0.0 else -1.0
                if current_vertical is None:
                    current_vertical = raw_vertical
                elif (
                    raw_vertical != current_vertical
                    and t - float(getattr(agent.state, "bangbang_last_switch_t", -np.inf))
                    >= self.case4_bangbang_min_dwell
                    and abs(vertical_signal) >= self.case4_bangbang_hysteresis
                ):
                    current_vertical = raw_vertical
                agent.state.bangbang_vertical_sign = current_vertical

                lateral_wave = self.case4_bangbang_lateral_scale * current_lateral
                vertical_wave = self.case4_bangbang_lateral_scale * current_vertical
                axial_sign = 1.0 if np.sin(2.0 * np.pi * t / (2.4 * period) + phase_role) >= 0.0 else -1.0
                axial_wave = self.case4_bangbang_axial_load * axial_sign
                load_x += fade * axial_wave
            else:
                raise ValueError(
                    "unsupported attack_pattern=%s" % self.attack_pattern
                )
            load_y = pn_load + fade * amp * lateral_wave / 9.8
        elif key == "case2" and "case2_sin_A" in self.paper_preset.files:
        ...
        if pattern != "nominal":
            load_z += fade * self.unseen_vertical_load_amp * vertical_wave
    ...
    def _trust_guide_actions(self, agent, world):
        ...
        """
        target = world.agents[agent.target]
        observed_pos = getattr(
            agent.state, "defender_last_observed_target_pos", None
        )
        observed_vel = getattr(
            agent.state, "defender_last_observed_target_vel", None
        )
        if observed_pos is None or observed_vel is None:
            observed_pos = target.state.p_pos
            observed_vel = target.state.p_vel
        rel = np.asarray(observed_pos - agent.state.p_pos, dtype=float)
        dist = max(float(np.linalg.norm(rel)), 1e-6)
        los = rel / dist
        own_vel = np.asarray(agent.state.p_vel, dtype=float)
        observed_los = getattr(
            agent.state, "defender_observed_los", None
        )
        if observed_los is not None:
            los = _unit(np.asarray(observed_los, dtype=float), los)
        observed_los_rate = np.asarray(
            getattr(
                agent.state,
                "defender_filtered_los_rate",
                np.zeros(3),
        ...
        observed_closing = float(
            getattr(
                agent.state,
                "defender_last_observed_closing",
                0.0,
            )
        )
        closing = max(-observed_closing, 0.0)
        nav_constant = self.trust_pn_navigation_constant
        pn_accel = nav_constant * closing * observed_los_rate
        ...
        # Positive n_z changes flight-path angle along this pitch-normal
        # direction.  Projecting onto the inertial z axis is only valid at
        # zero pitch and was a material 3-D guide error.
        pitch_hat = _unit(
            np.cross(vhat, lateral_hat),
            np.array([0.0, 0.0, 1.0]),
        )

        tgo_error = float(
            getattr(agent.state, "defender_last_tgo_error", 0.0)
        )
        tgo = dist / max(float(agent.state.v_vel[0]), 1.0)
        tgo_mean = tgo - tgo_error
        axial = np.clip(0.08 * tgo_error, -0.1, 1.0)
        pn = np.array(
            [
                axial,
                np.clip(np.dot(pn_accel, lateral_hat) / 9.81, -1.0, 1.0),
                np.clip(np.dot(pn_accel, pitch_hat) / 9.81, -1.0, 1.0),
            ],
            dtype=np.float32,
        )
        # Manuscript-consistent boundary probe: its lateral and vertical
        # signs reduce the *current velocity-to-LOS error*.  The previous
        # implementation copied the PN signs, which depend on unobserved
        # transverse target velocity and therefore did not match the stated
        # probe definition or the 17-D actor observation.
        desired_yaw = float(np.arctan2(los[1], los[0]))
        yaw_error = _wrap_pi(
            desired_yaw - float(agent.state.v_vel[2])
        )
        desired_pitch = float(
            np.arcsin(np.clip(los[2], -1.0, 1.0))
        )
        pitch_error = _wrap_pi(
            desired_pitch - float(agent.state.v_vel[1])
        )
        probe = np.array(
            [
                # Slow only a defender that is materially ahead of its
                # cooperative group.  At equality (in particular after its
                # partners have already hit), positive axial authority must
                # retain closing speed; the old ``else -0.1`` command could
                # strand the final group member.
                1.0 if tgo >= tgo_mean - 0.25 else -0.1,
                1.0 if yaw_error >= 0.0 else -1.0,
                1.0 if pitch_error >= 0.0 else -1.0,
            ],
            dtype=np.float32,
        )
        return pn, probe
