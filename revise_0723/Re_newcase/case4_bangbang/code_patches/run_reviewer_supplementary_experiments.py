...
def configure_case4_initial_state(env, seed: int) -> dict:
    """Keep the paper Case-2 geometry and change only attacker maneuver."""
    del env, seed
    return {"case4_geometry": "paper_case2_preset"}
...
def _build_raw_args(args):
    return SimpleNamespace(
        ...
        case3_terminal_frequency_scale=args.case3_terminal_frequency_scale,
        case3_terminal_chirp_rate=args.case3_terminal_chirp_rate,
        case3_terminal_frequency_cap=args.case3_terminal_frequency_cap,
        case4_bangbang_period=args.case4_bangbang_period,
        case4_bangbang_min_dwell=args.case4_bangbang_min_dwell,
        case4_bangbang_hysteresis=args.case4_bangbang_hysteresis,
        case4_bangbang_axial_load=args.case4_bangbang_axial_load,
        case4_bangbang_lateral_scale=args.case4_bangbang_lateral_scale,
    )
...
def build_environment_and_policy(args):
    ...
    source_case = "case2" if args.case in {"case3", "case4"} else args.case
    ...
    if args.case in {"case3", "case4"}:
        cfg.defender_guidance_base_gain = 2.6
        cfg.defender_guidance_tau = 0.35
        cfg.defender_guidance_lead = 1.70
        cfg.defender_residual_scale = args.case34_defender_residual_scale
        cfg.defender_sync_speed_gain = args.case34_defender_sync_speed_gain
        cfg.defender_command_lag_tau = 0.40
    ...
    cfg.case3_terminal_frequency_scale = args.case3_terminal_frequency_scale
    cfg.case3_terminal_chirp_rate = args.case3_terminal_chirp_rate
    cfg.case3_terminal_frequency_cap = args.case3_terminal_frequency_cap
    cfg.case4_bangbang_period = args.case4_bangbang_period
    cfg.case4_bangbang_min_dwell = args.case4_bangbang_min_dwell
    cfg.case4_bangbang_hysteresis = args.case4_bangbang_hysteresis
    cfg.case4_bangbang_axial_load = args.case4_bangbang_axial_load
    cfg.case4_bangbang_lateral_scale = args.case4_bangbang_lateral_scale
    if args.cpu_eval:
        cfg.cuda = False
...
def evaluate(args):
    ...
    hit_event_rows: list[dict] = []
    success_episode_rows: list[dict] = []
    selected_trajectory_saved = False
    ...
    for episode in range(args.episodes):
        ...
        unseen_geometry = (
            ...
            if args.case == "case3"
            else configure_case4_initial_state(env, episode_seed)
            if args.case == "case4"
            else {}
        )
        ...
        defender_pos_history: list[np.ndarray] = []
        attacker_pos_history: list[np.ndarray] = []
        tgo_history: list[np.ndarray] = []
        ...
        for step in range(args.max_steps):
            ...
            defender_pos_history.append(
                np.asarray([a.state.p_pos.copy() for a in defenders])
            )
            attacker_pos_history.append(
                np.asarray([a.state.p_pos.copy() for a in attackers])
            )
            tgo_values = []
            for agent in defenders:
                target = env.world.agents[agent.target]
                ...
                tgo_values.append(distance / max(float(agent.state.v_vel[0]), 1.0))
            tgo_history.append(np.asarray(tgo_values, dtype=float))
                    ...
                    hit_event_rows.append(
                        {
                            ...
                            "time": float(agent.state.hit_time),
                            "dist_to_target": distance,
                            "defender_pos_x": float(agent.state.p_pos[0]),
                            "defender_pos_y": float(agent.state.p_pos[1]),
                            "defender_pos_z": float(agent.state.p_pos[2]),
                        }
                    )
        ...
        if mission_success:
            valid_spreads = [
                float(m["arrival_spread_s"])
                for m in per_target_metrics
                if np.isfinite(float(m["arrival_spread_s"]))
            ]
            success_episode_rows.append(
                {
                    ...
                    "episode": episode_id,
                    "seed": episode_seed,
                    "mean_sync_spread": float(np.mean(valid_spreads)) if valid_spreads else float("nan"),
                    "max_sync_spread": float(np.max(valid_spreads)) if valid_spreads else float("nan"),
                    "targets_covered": int(np.count_nonzero(target_covered)),
                    "complete_groups": int(np.count_nonzero(group_complete)),
                    "coordinated_groups": int(np.count_nonzero(group_coordinated)),
                }
            )
            if not selected_trajectory_saved:
                case_outdir = outdir / args.case
                case_outdir.mkdir(parents=True, exist_ok=True)
                np.savez(
                    case_outdir / f"{args.case}_selected_episode.npz",
                    rep_def=np.asarray(defender_pos_history, dtype=np.float32),
                    rep_att=np.asarray(attacker_pos_history, dtype=np.float32),
                    rep_ctrl=np.asarray(load_history, dtype=np.float32),
                    rep_tgo=np.asarray(tgo_history, dtype=np.float32),
                    assignment=np.asarray(assignment, dtype=np.int32),
                    seed=np.asarray([episode_seed], dtype=np.int64),
                    episode=np.asarray([episode_id], dtype=np.int64),
                )
                selected_trajectory_saved = True
    ...
    case_outdir = outdir / args.case
    case_outdir.mkdir(parents=True, exist_ok=True)
    _write_csv(case_outdir / f"{args.case}_hit_events.csv", hit_event_rows)
    _write_csv(
        case_outdir / f"{args.case}_success_episodes.csv",
        success_episode_rows,
    )
    _write_csv(case_outdir / f"{args.case}_tailchase_stats.csv", [])
    ...
    summary = {
        ...
        "assignment_mode": args.assignment_mode,
        "source_policy_case": (
            "case2" if args.case in {"case3", "case4"} else args.case
        ),
        "unseen_geometry": (
            "wider staggered-altitude shell with biased inbound headings"
            if args.case == "case3"
            else "paper case2 geometry with bang-bang attacker maneuver"
            if args.case == "case4"
            else None
        ),
        "attacker_maneuver": (
            ...
            if args.case == "case3"
            else "finite-dwell bounded bang-bang lateral/vertical evasion"
            if args.case == "case4"
            else None
        ),
...
def parse_args():
    ...
    parser.add_argument("--case34_defender_residual_scale", type=float, default=0.1)
    parser.add_argument("--case34_defender_sync_speed_gain", type=float, default=2.0)
    ...
    if args.case == "case4" and args.attack_pattern != "bangbang":
        parser.error(
            "Case 4 requires --attack_pattern bangbang"
        )
