...
run_root=/home/a2rl/reviewer_art_mappo_paper_aligned_20260728_temporally_coherent_v10
python_bin=/home/a2rl/miniconda3/envs/rlgpu/bin/python
monitor_log="${run_root}/checkpoint_monitor.log"

while [[ ! -f "${run_root}/logs/full_case1_seed9261.pid" ||
         ! -f "${run_root}/logs/full_case2_seed9261.pid" ]]; do
    sleep 20
done
case1_pid="$(tr -d '[:space:]' <"${run_root}/logs/full_case1_seed9261.pid")"
case2_pid="$(tr -d '[:space:]' <"${run_root}/logs/full_case2_seed9261.pid")"
printf '[%s] monitoring v10 pids %s %s\n' \
    "$(date --iso-8601=seconds)" "${case1_pid}" "${case2_pid}" >>"${monitor_log}"

cd "${project_root}"
for update in 5 15 30 60 100 150; do
    tag="$(printf '%04d' "${update}")"
    checkpoint1="${run_root}/training/full/case1/seed9261/models/checkpoint_update_${tag}.pt"
    checkpoint2="${run_root}/training/full/case2/seed9261/models/checkpoint_update_${tag}.pt"
    while [[ ! -f "${checkpoint1}" || ! -f "${checkpoint2}" ]]; do
        if ! kill -0 "${case1_pid}" 2>/dev/null &&
           ! kill -0 "${case2_pid}" 2>/dev/null; then
            printf '[%s] v10 exited before checkpoint %s\n' \
    ...
    for case_name in case1 case2; do
        ...
        mkdir -p "${one}"
        cp "${run_root}/training/full/${case_name}/seed9261/models/checkpoint_update_${tag}.pt" "${one}/"
