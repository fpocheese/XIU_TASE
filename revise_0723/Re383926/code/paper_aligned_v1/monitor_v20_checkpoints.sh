...
run_root=/home/a2rl/reviewer_art_mappo_paper_aligned_20260728_sensor_masked_v20
python_bin=/home/a2rl/miniconda3/envs/rlgpu/bin/python
monitor_log="${run_root}/checkpoint_monitor.log"
while [[ ! -f "${run_root}/logs/full_case1_seed9661.pid" ||
         ! -f "${run_root}/logs/full_case2_seed9661.pid" ]]; do sleep 20; done
case1_pid="$(tr -d '[:space:]' <"${run_root}/logs/full_case1_seed9661.pid")"
case2_pid="$(tr -d '[:space:]' <"${run_root}/logs/full_case2_seed9661.pid")"
printf '[%s] monitoring v20 pids %s %s\n' "$(date --iso-8601=seconds)" \
    "${case1_pid}" "${case2_pid}" >>"${monitor_log}"
cd "${project_root}"
for update in 5 10 15 20 30 45 60 100 150; do
    tag="$(printf '%04d' "${update}")"
    checkpoint1="${run_root}/training/full/case1/seed9661/models/checkpoint_update_${tag}.pt"
    checkpoint2="${run_root}/training/full/case2/seed9661/models/checkpoint_update_${tag}.pt"
    ...
    for case_name in case1 case2; do
        seed=99981; [[ "${case_name}" == case2 ]] && seed=99991
        one="${out}/${case_name}_checkpoint_only"; mkdir -p "${one}"
        cp "${run_root}/training/full/${case_name}/seed9661/models/checkpoint_update_${tag}.pt" "${one}/"
