...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for agent in self.world.policy_agents:
            ...
            if action_dim == 3:
                # Paper Eq. (actor_policy): axial load has the asymmetric
                # envelope [-0.1, 1], while both normal-load channels use
                # [-1, 1].  Keep the Gym space consistent with the dynamics.
                action_low = np.array([-0.1, -1.0, -1.0], dtype=np.float32)
    ...
    def step(self, action_n, step=0):
        ...
        for agent in self.agents123:
            ...
            info["all_defenders_done"] = bool(
                all(a.doneflag for a in self.agents123)
            )
            info_n.append(info)
