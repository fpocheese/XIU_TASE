...
class Scenario(BaseScenario):
    ...
    def _paper_key(self):
        # Case 3 is a frozen-policy generalization condition.  It starts from
        # the Case-2 paper preset only as a reproducible state template; the
        # evaluator then applies the preregistered Case-3 geometry transform
        # before IDBO assignment, and the attacker uses a distinct hybrid
        # maneuver below.
        return "case2" if self.case in {"case2", "case3"} else "case1"
        ...
        if pattern != "nominal":
            ...
            if pattern == "chirp":
                ...
                if t < switch_1:
                    ...
                    lateral_wave = np.sin(wave_phase)
                    vertical_wave = np.sin(
                        0.67 * wave_phase + 0.5 * phase
                    )
                    axial_wave = 0.16 * np.sin(
                        0.31 * wave_phase + phase
                    )
