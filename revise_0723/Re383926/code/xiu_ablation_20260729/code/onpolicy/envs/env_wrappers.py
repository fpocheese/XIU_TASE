...
class DummyVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        # print(self.actions)
        # print(self.envs)
        results = [env.step(a,env) for (a, env) in zip(self.actions, self.envs)]
        obs_list, rews_list, dones_list, infos_list = zip(*results)
        # Handle inhomogeneous reward/info shapes from custom envs
        try:
        ...
        try:
            ...
            rews = np.array(rews_list, dtype=object)
        try:
            dones = np.array(dones_list)
        except ValueError:
            dones = np.array(dones_list, dtype=object)
        try:
            infos = np.array(infos_list)
        except ValueError:
            infos = np.array(infos_list, dtype=object)
        # print(dones)
