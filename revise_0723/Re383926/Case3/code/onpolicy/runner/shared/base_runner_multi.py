...
class MultiAlgoRunner(object):
    ...
    def __init__(self, config, algo_name="MAPPO"):
        ...
        if self.model_dir is not None and str(self.model_dir).strip() != "":
            self.restore()
    ...
    def restore(self):
        ...
        self.policy.actor.load_state_dict(torch.load(actor_path, map_location=self.device))
        if os.path.exists(critic_path):
            self.policy.critic.load_state_dict(torch.load(critic_path, map_location=self.device))
        print(f"[WARMSTART] loaded model from {self.model_dir}")
