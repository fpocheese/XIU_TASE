...
import torch
from onpolicy.config_advanced import get_advanced_config, print_advanced_config_info
...
def main(args):
    ...
    run_dir = Path(os.path.split(os.path.dirname(os.path.abspath(__file__)))[0] + "/results") / \
              all_args.env_name / all_args.scenario_name / "advanced_mappo" / all_args.experiment_name
    ...
    if all_args.use_wandb:
        run = wandb.init(
            ...
            job_type="training",
            reinit=True,
            tags=["advanced_mappo", "attention", "dual_clip"]
        )
    else:
        if not run_dir.exists():
            curr_run = 'run1'
