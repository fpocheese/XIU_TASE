#!/usr/bin/env python
"""
Simple Spread V2 多算法对比训练 — 确保全部收敛
...
# ====================== 环境 ======================
def make_env(num_agents=3, num_landmarks=3, episode_length=50, seed=0):
    from onpolicy.envs.mpe.simple_spread_v2 import SimpleSpreadEnvV2
    ...
    class VecEnv:
        ...
        def step(self, actions):
            ...
            if np.all(dones): obs = np.array([self.env.reset()])
            return obs, rews, dones, infos
        def close(self): self.env.close()

    env = SimpleSpreadEnvV2(num_agents=num_agents, num_landmarks=num_landmarks,
...
class OnPolicyRunner:
    def __init__(self, args, envs, algo_name, device, num_agents):
        self.args = args; self.envs = envs; self.algo_name = algo_name
        self.device = device; self.num_agents = num_agents
        self.ep_len = args.episode_length; self.hidden_size = args.hidden_size
        self.recurrent_N = args.recurrent_N
        self.use_cent_V = args.use_centralized_V
        if algo_name in ["IPPO", "IA2C", "IQL"]: self.use_cent_V = False
        self._setup(algo_name)
        share_sp = envs.share_observation_space[0] if self.use_cent_V else envs.observation_space[0]
        self.buffer = SharedReplayBuffer(args, num_agents, envs.observation_space[0], share_sp, envs.action_space[0])

    def _setup(self, algo):
        obs_sp = self.envs.observation_space[0]; act_sp = self.envs.action_space[0]
    ...
    def run(self, num_episodes):
        ...
        for ep in range(num_episodes):
            ...
            if (ep+1) % 100 == 0 or ep == 0:
                print(f"  [{self.algo_name}] Ep {ep+1}/{num_episodes} | R={r:.2f} | Avg100={np.mean(reward_curve[-100:]):.2f}")
        return reward_curve

    def _episode(self):
        obs = self.envs.reset()
        if self.use_cent_V:
            so = obs.reshape(1,-1); so = np.expand_dims(so,1).repeat(self.num_agents,axis=1)
        else: so = obs
        self.buffer.share_obs[0] = so.copy(); self.buffer.obs[0] = obs.copy()
        ep_r = 0.0
        for step in range(self.ep_len):
            vals, acts, alp, rs, rsc = self._collect(step)
            obs, rews, dones, infos = self.envs.step(acts)
            rews = np.array(rews, dtype=np.float32)
            if rews.ndim == 2: rews = rews[:,:,np.newaxis]
        ...
        self._compute()
        self.trainer.prep_training(); self.trainer.train(self.buffer); self.buffer.after_update()
        return ep_r
    ...
    def _collect(self, step):
        ...
        v,a,alp,rs,rsc = self.trainer.policy.get_actions(
            np.concatenate(self.buffer.share_obs[step]), np.concatenate(self.buffer.obs[step]),
            np.concatenate(self.buffer.rnn_states[step]), np.concatenate(self.buffer.rnn_states_critic[step]),
            np.concatenate(self.buffer.masks[step]))
        return (np.array(np.split(_t2n(v),1)), np.array(np.split(_t2n(a),1)),
                np.array(np.split(_t2n(alp),1)), np.array(np.split(_t2n(rs),1)),
...
class MADDPGRunner:
    def __init__(self, args, envs, device, num_agents):
        self.envs = envs; self.device = device; self.num_agents = num_agents
        self.ep_len = args.episode_length; self.algo_name = "MADDPG"
    ...
    def run(self, num_episodes):
        ...
        for ep in range(num_episodes):
            ...
            curve.append(r)
            if (ep+1)%100==0 or ep==0:
                print(f"  [MADDPG] Ep {ep+1}/{num_episodes} | R={r:.2f} | Avg100={np.mean(curve[-100:]):.2f}")
        return curve

    def _episode(self):
        obs = self.envs.reset()[0]; share = np.tile(obs.flatten(),(self.num_agents,1))
        ep_r = 0.0; self.maddpg.prep_rollout()
        for _ in range(self.ep_len):
            ...
            nobs = nobs[0]; rews_a = np.array(rews[0],dtype=np.float32).reshape(self.num_agents,1)
            ...
            self.maddpg.prep_training(); self.maddpg.update()
        return ep_r

# ====================== 配置 ======================
def make_args(episode_length=50):
    parser = get_config()
    args = parser.parse_known_args([])[0]
    
    # 核心超参数 — 精心调优确保收敛
    args.episode_length = episode_length
    args.hidden_size = 128          # 略大的网络有助于注意力机制发挥
    args.layer_N = 1
    args.lr = 5e-4                  # 经典学习率
    args.critic_lr = 5e-4
    args.ppo_epoch = 10             # 标准PPO epoch
    args.clip_param = 0.2           # 标准clip
    ...
    # Advanced-MAPPO 专用 — 关闭warmup，让它从第一个episode就开始学习
    args.use_value_warmup = False   # 关闭value warmup!
    args.warmup_episodes = 0
    args.use_dual_clip = True
    args.dual_clip_param = 3.0
    args.use_adaptive_kl = True
    args.target_kl = 0.01           # 更保守的KL目标
...
# ====================== 训练 ======================
def train_one(algo, seed, num_episodes=1000, save_dir=None):
    print(f"\n{'='*60}\n  {algo} | seed={seed} | {num_episodes} episodes\n{'='*60}")
    torch.manual_seed(seed); torch.cuda.manual_seed_all(seed); np.random.seed(seed)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    
    args = make_args(episode_length=50)
    envs = make_env(num_agents=3, num_landmarks=3, episode_length=50, seed=seed)
    
    t0 = time.time()
    if algo == "MADDPG":
        runner = MADDPGRunner(args, envs, device, 3)
    else:
        runner = OnPolicyRunner(args, envs, algo, device, 3)
    curve = runner.run(num_episodes)
    dt = time.time() - t0
    envs.close()
    
    if save_dir is None:
        save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'simple_spread_v2')
    os.makedirs(save_dir, exist_ok=True)
    f = os.path.join(save_dir, f"{algo}_seed{seed}_rewards.npy")
    np.save(f, np.array(curve))
    print(f"  Done in {dt/60:.1f}min | Final100={np.mean(curve[-100:]):.2f} | Saved: {f}")
    return curve
...
def plot_comparison(save_dir):
    ...
    for algo, color, lw in zip(algos, colors, lws):
        ...
        for s in seeds:
            ...
            if os.path.exists(f): curves.append(np.load(f))
        if not curves: print(f"  [skip] {algo}"); continue
        ...
        mean = sm.mean(axis=0); std = sm.std(axis=0)
        x = np.arange(len(mean))
        
        ax.plot(x, mean, label=algo, color=color, linewidth=lw, 
                linestyle='-' if algo=='Advanced-MAPPO' else '-',
                zorder=10 if algo=='Advanced-MAPPO' else 5)
        ax.fill_between(x, mean-std, mean+std, color=color, alpha=0.12)
        
        algo_stats[algo] = {
            'final': data[:,-100:].mean(),
            'final_std': np.mean([d[-100:].mean() for d in data]),
            'final_seeds_std': np.std([d[-100:].mean() for d in data]),
            'best': data.mean(axis=0).max(),
        }
    
    ax.set_xlabel('Episode', fontsize=15)
    ax.set_ylabel('Episode Reward', fontsize=15)
    ax.set_title('Simple Spread: Multi-Algorithm Comparison (3 agents, 3 landmarks)', fontsize=16)
    ...
    for algo in valid:
        ...
        fm = data[:,-100:].mean()
        fs = np.std([d[-100:].mean() for d in data])
        best = mean_curve.max()
        ...
        for idx, val in enumerate(sm_mean):
            if fm >= 0 and val >= target: conv = str(idx); break
            elif fm < 0 and val >= target: conv = str(idx); break
        
        rows.append([algo, f'{fm:.2f} ± {fs:.2f}', f'{best:.2f}', conv])
    ...
    for a in valid:
        ...
        print(f"  {a:<20} {s['final']:.2f} ± {s['final_seeds_std']:.2f}       {s['best']:.2f}{marker}")
...
def main():
    ...
    if cmd.plot_only:
        plot_comparison(save_dir); return
    ...
    total = len(algos)*len(seeds); idx = 0
    t0 = time.time()
    for algo in algos:
        for seed in seeds:
            idx += 1
            print(f"\n{'#'*60}\n  [{idx}/{total}] {algo} seed={seed}\n{'#'*60}")
            train_one(algo, seed, num_episodes=cmd.num_episodes, save_dir=save_dir)
