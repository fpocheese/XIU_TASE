...
$N_D$ and the total per-round cost grows linearly with $N_D$. Empirically, the number of
consensus rounds is governed by the communication-graph diameter $D$ and the message delay
rather than by $N_D$: it stays nearly constant as the swarm scales, and it grows only linearly
with both the delay and $D$, consistent with the $O(N\cdot D)$-round bound. A bounded delay
therefore slows, but does not prevent, convergence of the distributed consensus.
}
