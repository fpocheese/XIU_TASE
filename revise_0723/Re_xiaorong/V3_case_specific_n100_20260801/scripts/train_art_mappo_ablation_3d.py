...
def add_ablation_arguments(parser):
    ...
    parser.add_argument(
        "--train_initial_perturbation_scale",
        type=float,
        default=0.0,
        help=(
            "Training-only initial-state perturbation scale. Zero preserves "
            "the legacy fixed paper preset; one matches the audited evaluator."
        ),
    )
    parser.add_argument(
        "--train_initial_perturbation_seed",
        type=int,
        default=700001,
        help="Independent reproducible RNG seed for training initial states.",
    )
    parser.add_argument(
...
def validate_args(args):
    ...
    if args.train_initial_perturbation_scale < 0.0:
        raise ValueError("train_initial_perturbation_scale must be nonnegative")
    if args.train_initial_perturbation_seed < 0:
        raise ValueError("train_initial_perturbation_seed must be nonnegative")
