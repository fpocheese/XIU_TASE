...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        # Optional training-only initial-state randomization.  It is disabled by
        # default so that legacy experiments are bit-for-bit unaffected.  When
        # enabled, the distribution matches the physical perturbation used by
        # the audited Monte-Carlo evaluator (position, heading, and speed only).
        self.train_initial_perturbation_scale = float(
            _arg(args, "train_initial_perturbation_scale", 0.0)
        )
        self.train_initial_perturbation_seed = int(
            _arg(args, "train_initial_perturbation_seed", 700001)
        )
        self.train_initial_perturbation_rng = np.random.default_rng(
            self.train_initial_perturbation_seed
        )
        self.train_initial_perturbation_reset_count = 0
        ...
        self.physical_episode_horizon_steps = int(
    ...
    def reset_world(self, world):
        ...
        if self.paper_preset is not None:
            ...
            if self.train_initial_perturbation_scale > 0.0:
                self._perturb_paper_initial_state(defenders, attackers)
            return
    ...
    def _perturb_paper_initial_state(self, defenders, attackers):
        """Apply the evaluator's initial-state perturbation during training.

        The random stream is local to this Scenario instance and seeded from an
        explicit command-line argument, so each training run is reproducible.
        No dynamics, reward, terminal condition, or target assignment is changed.
        """
        rng = self.train_initial_perturbation_rng
        scale = self.train_initial_perturbation_scale

        for agent in defenders:
            agent.state.p_pos[:2] += rng.normal(0.0, 8.0 * scale, size=2)
            yaw_delta = rng.normal(0.0, 0.045 * scale)
            speed_scale = np.clip(
                rng.normal(1.0, 0.025 * scale), 0.90, 1.10
            )
            velocity = np.asarray(agent.state.p_vel, dtype=float).copy()
            cos_yaw, sin_yaw = np.cos(yaw_delta), np.sin(yaw_delta)
            velocity[:2] = np.array(
                [
            ...
            agent.state.p_vel = velocity * speed_scale
            agent.state.v_vel = self._vel_to_flight_state(agent.state.p_vel)

        for agent in attackers:
            radial_scale = np.clip(
                rng.normal(1.0, 0.018 * scale), 0.94, 1.06
            )
            angle_delta = rng.normal(0.0, 0.025 * scale)
            position = np.asarray(agent.state.p_pos, dtype=float).copy()
            cos_ang, sin_ang = np.cos(angle_delta), np.sin(angle_delta)
            position[:2] = radial_scale * np.array(
                [
                    cos_ang * position[0] - sin_ang * position[1],
                    sin_ang * position[0] + cos_ang * position[1],
                ]
            )
            position[2] += rng.normal(0.0, 3.0 * scale)
            agent.state.p_pos = position

            speed = max(float(np.linalg.norm(agent.state.p_vel)), 1e-6)
            speed *= np.clip(
                rng.normal(1.0, 0.020 * scale), 0.92, 1.08
            )
            direction = _unit(
                self.protected_asset - agent.state.p_pos,
                np.array([0.0, 0.0, -1.0]),
            )
            agent.state.p_vel = speed * direction
            agent.state.v_vel = self._vel_to_flight_state(agent.state.p_vel)

        self.train_initial_perturbation_reset_count += 1
    ...
    def done_callback(self, agent, world):
        ...
        # The historical training runner reset every 1500 simulation steps.
        # The reviewer runner uses shorter PPO buffers that may straddle a
        # physical engagement, so retain the same 75-s physical horizon
        # explicitly and mark a non-intercepted defender as a timeout.
        if (
            self.physical_episode_horizon_steps > 0
            and agent.state.timestep
            >= self.physical_episode_horizon_steps * world.dt
        ):
            agent.doneflag = True
            agent.state.actual_hit = False
            agent.state.timeover = True
            return True
    ...
    def info(self, agent, world):
        """Training-only guide metadata for trust-modulated exploration.

        The policy action is a residual around the verified deterministic
        guidance law.  Consequently, the PN/base guide is represented by a
        zero residual.  The probe is a feasible boundary residual whose signs
        reduce the instantaneous yaw and flight-path-to-LOS errors.  These
        fields do not alter the environment dynamics or reward.
        """
        target = world.agents[agent.target]
        rel = target.state.p_pos - agent.state.p_pos
        dist_xy = max(float(np.linalg.norm(rel[:2])), 1e-9)
        desired_yaw = np.arctan2(rel[1], rel[0])
        desired_pitch = np.arctan2(rel[2], dist_xy)
        yaw_error = _wrap_pi(desired_yaw - float(agent.state.v_vel[2]))
        pitch_error = desired_pitch - float(agent.state.v_vel[1])
        probe_action = np.array(
            [
                0.20,
